...
export default defineConfig({
  ...
  schemaFilter: ['public'],
  /**
   * Every table declared in src/database/schema. Anything missing here is
   * invisible to `push:pg`, so schema edits to it silently never reach the
   * database.
   *
   * Safe to keep complete because MigrationService.ensureDatabaseInvariants()
   * runs immediately AFTER push and recreates the partial indexes that
   * drizzle-kit 0.20 cannot express (bookings_live_unit_uq and friends). Push
   * drops them; the invariant step puts them straight back. Do not remove that
   * step without narrowing this list again.
   *
   * To regenerate:
